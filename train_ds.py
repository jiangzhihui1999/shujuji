...
# 学习率衰减
# learning_rate_decay = [500, 750]表示epoch为500和750时调整一次学习率
lr_decay = torch.optim.lr_scheduler.MultiStepLR(opt, para.learning_rate_decay)

# 深度监督衰减系数
alpha = para.alpha

pathname = "./module/" + str(Time.localtime().tm_year) + str(Time.localtime().tm_mon).zfill(2)\
           + str(Time.localtime().tm_mday).zfill(2)
if not os.path.exists(pathname):  # 保存模型参数的文件夹
...
for epoch in range(para.Epoch):
    ...
    if epoch % 50 is 0 and epoch is not 0:
        ...
        torch.save(net.state_dict(), '{}/net{}-{:.3f}-{:.3f}.pth'.format(pathname, epoch, loss, mean_loss))
