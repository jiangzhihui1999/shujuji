...
class Dataset(dataset):
    def __init__(self, ct_dir, seg_dir):
        self.ct_list = os.listdir(ct_dir)  # 返回dir下的文件名称？
        self.seg_list = list(
            map(lambda x: x.replace('volume', 'segmentation').replace('.nii', '.nii.gz'), self.ct_list))
        # self.seg_list = list(
        #     map(lambda x: x.replace('volume', 'segmentation'), self.ct_list))
        self.ct_list = list(map(lambda x: os.path.join(ct_dir, x), self.ct_list))  # 返回dir下的文件的绝对路径？
